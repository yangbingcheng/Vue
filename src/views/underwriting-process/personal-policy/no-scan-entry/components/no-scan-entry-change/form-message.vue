<template>
  <div class="main">
    <el-main>
      <p><span @click="isShowCompact">{{ shapeCompact }}</span>投保人信息(客户号:<el-input v-model="clientNumber" style="width:200px;margin:5px;" size="mini"></el-input><el-button @click="queryBtn" type="success" style="margin:5px;" size="mini">查询</el-button>首次投保客户无需填写客户号)</p>
      <el-form :model="formData" :rules="rules" ref="formData" label-width="130px" class="demo-ruleForm" label-position="right" v-if="flagCompact">
        ...
        <el-row type="flex" class="row-bg">
          ...
          <el-col :span="8">
            <el-form-item  label="系被保人的" :required="false" prop="policy" size="small">
        ...
        <el-row>
          <el-col :span="16">
            <el-form-item  label="是否拥有公费医疗、社会医疗保险" :required="false" prop="isPolicy" size="small" label-width="250px">
    ...
    <el-main>
      ...
      <el-form :model="formDatas" :rules="rules" ref="formDatas" label-width="130px" class="demo-ruleForm" label-position="right" v-if="flagOther">
        ...
        <el-row type="flex" class="row-bg">
          <el-col :span="16">
            <el-form-item  label="不能提供居民国（地区）纳税人识别号原因:" :required="false" prop="job" size="small" label-width="300px">
                     <el-select v-model="formData.job" filterable placeholder="请选择" @change="selectChange1" :disabled="false" style="">
                        <el-option
                          v-for="item in selectData"
                          :key="item.value"
